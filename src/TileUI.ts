...
export class TileUI {
	...
	addButton(label: string, callback: () => void): Controller<() => void> {
		// ButtonController はオブジェクトのプロパティにバインドする設計のため、
		// ボタン用の一時オブジェクトを生成して __callback プロパティに紐づける
	...
	dispose(): void {
		...
		// サブフォルダのコントローラを内側から先に dispose する
		for (const folder of this.folders) {
			folder.dispose();
		}
		this.folders = [];
	...
	private setupColumns(columns: number | ColumnOption | undefined, dock?: DockPosition): void {
		...
		if (dock === 'left' || dock === 'right') {
			...
			// 利用可能な幅から最大列数を求め、行数を最小化しつつ各行のアイテム数を揃える
